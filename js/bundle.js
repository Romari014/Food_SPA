...
function form(formSelector, modalTimerId) {
  ...
  function bindPostData(form) {
    form.addEventListener('submit', e => {
      ...
      form.insertAdjacentElement('afterend', statusMessage); // request.setRequestHeader('Content-type', 'multipart/form-data');
...
function slider({
  ...
  const indicatorDot = document.createElement('ol'),
        dots = [];
  indicatorDot.classList.add('carousel-indicators');
...
function timer(id, deadline) {
  function getTimeRemaning(endtime) {
    const t = Date.parse(endtime) - Date.parse(new Date()),
          //Date.parse Строковое представление даты 
    days = Math.floor(t / (1000 * 60 * 60 * 24)),
          //Сщитаем количество дней.  Math.floor(округляет числа)
...
const postData = async (url, data) => {
  //async , await делает синхронный код вместо асинхронного
...
__webpack_require__(/*! es6-promise */ "./node_modules/es6-promise/dist/es6-promise.js").polyfill(); //test
...
document.addEventListener('DOMContentLoaded', () => {
  const modalTimerId = setTimeout(() => (0,_modules_modal__WEBPACK_IMPORTED_MODULE_2__.showModal)('.modal', modalTimerId), 300000);
  (0,_modules_tabs__WEBPACK_IMPORTED_MODULE_1__.default)('.tabheader__item', '.tabcontent', '.tabheader__items', 'tabheader__item_active');
  (0,_modules_modal__WEBPACK_IMPORTED_MODULE_2__.default)('.btn[data-open]', '.modal', modalTimerId);
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_3__.default)('.timer', '2021-08-28');
